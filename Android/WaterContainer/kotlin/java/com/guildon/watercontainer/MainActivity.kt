...
class MainActivity : Activity() {
	...
	/**
	 * Analyse the wall configuration, and find spaces where the water would fill up
	 */
	private fun analyse() {
		...
		for (height: Int in lowest..highest) {
			...
			for (j in 0 until maxWidth) {
				...
				// If we find a wall of the requested height
		...
		textVolume.text = "Calculated volume is " + volume.toString()
	...
	private fun search(column: Int, height: Int, direction: Int): Int {
		when (direction) {
			// Searching left
			1 -> for (found in column + 1 until maxWidth) {
				if (heightmap[found] > height)
					return found
			}

			// Searching right
			-1 -> for (found in column - 1 downTo 0) {
				if (heightmap[found] > height)
					return found
			}
